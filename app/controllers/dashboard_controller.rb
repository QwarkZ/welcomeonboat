class DashboardController < ApplicationController
  def index
    ...
    @my_future_bookings = Booking.joins(:boat).where("renter_id = ?", current_user.id).where("start_date > ?", Time.now).where("booking_status = ?", "Validée").order("start_date")
    @my_other_bookings = Booking.joins(:boat).where("renter_id = ?", current_user.id).where("start_date < ?", Time.now).order("start_date")
